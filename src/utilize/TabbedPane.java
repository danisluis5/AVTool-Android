  ...
  private class TabCloseUI implements MouseListener, MouseMotionListener
    {
        ...
        public void mouseReleased(MouseEvent me) { if (closeUnderMouse(me.getX(), me.getY())) {
            boolean isToCloseTab = TabbedPane.this.tabAboutToClose(this.selectedTab);
          ...
          if (this.tabbedPane.getTabCount() > 0)
            if (closeUnderMouse(this.meX, this.meY)) {
              ...
              if (this.selectedTab > -1)
                this.tabbedPane.setToolTipTextAt(this.selectedTab, "Click vào đóng tab lại!");
            }
            else {
        ...
        private boolean mouseOverTab(int x, int y) {
          ...
          for (int j = 0; j < tabCount; j++)
            if (this.tabbedPane.getBoundsAt(j).contains(this.meX, this.meY)) {
              this.selectedTab = j;
              this.closeX = (this.tabbedPane.getBoundsAt(j).x + this.tabbedPane.getBoundsAt(j).width - 8 - 5 - 2);
              this.closeY = (this.tabbedPane.getBoundsAt(j).y + 5 + 2);
              return true;
            }
          return false;
        }
